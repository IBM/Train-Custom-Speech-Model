...
Go to the [ezDI webpage](https://www.ezdi.com/open-datasets/) and download the medical dictation dataset into the data directory. The data includes the audio and the verbatim transcription done by human.

The transcription files are in the rtf format.  We need to convert them to plain text format. 

```
pip install pyth 
python convert_rtf.py *.rtf
```
...
```
zip audio-test.zip -xi `ls [1-5].wav`
zip audio-set1.zip -xi `ls [6-9].wav [1-7][0-9].wav`
zip audio-set2.zip -xi `ls [8-9][0-9].wav 1[0-6][0-9].wav`
zip audio-set3.zip -xi `ls 1[7-9][0-9].wav 2[0-4][0-9].wav`
```
